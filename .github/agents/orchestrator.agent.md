...
<available_agents>
researcher, planner, implementer, browser-tester, devops, reviewer, documentation-writer
</available_agents>
...
- Phase 1: Research (if no research findings):
  ...
  - Delegate to multiple `researcher` instances concurrent (one per focus_area) with: objective, focus_area, plan_id
  - Wait for all researchers to complete
- Phase 2: Planning:
  - Verify research findings exist in `docs/plan/{plan_id}/research_findings_*.yaml`
  - Delegate to `planner`: objective, plan_id
...
- Phase 3: Execution Loop:
  ...
  - Delegate to worker agents via `runSubagent` (up to 4 concurrent):
    * implementer/browser-tester/devops/documentation-writer: Pass task_id, plan_id
    * `reviewer`: Pass task_id, plan_id (if requires_review=true or security-sensitive)
  ...
  - Synthesize: Update `plan.yaml` status based on results:
    ...
    * FAILURE/NEEDS_REVISION → If fixable: delegate to `implementer` (task_id, plan_id); If requires replanning: delegate to `planner` (objective, plan_id)
...
- Phase 4: Completion (all tasks completed):
  ...
  - FINAL: Present comprehensive summary via `walkthrough_review`
    * If userfeedback indicates changes needed → Route updated objective, plan_id to `researcher` (for findings changes) or `planner` (for plan changes)
</workflow>
...
- Failure handling:
  * Task failure (fixable): Delegate to implementer with task_id, plan_id
  * Task failure (requires replanning): Delegate to planner with objective, plan_id
